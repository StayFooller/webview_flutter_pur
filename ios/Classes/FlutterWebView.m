...
#import <webflutter/webflutter.h>
...
typedef void(^XBRouterBlock)(NSDictionary *dic);
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super init]) {
    ...
    [self updateAutoMediaPlaybackPolicy:args[@"autoMediaPlaybackPolicy"]
      ...
      FLTWKNavigationDelegate *navDelegate = [[FLTWKNavigationDelegate alloc] initWithChannel:_channel];
    _webView = [[FLTWKWebView alloc] initWithFrame:frame configuration:configuration];
      [_webView addJavascriptObject:navDelegate namespace:nil];
//        [_webView setDebugMode:true];
      [_webView customJavascriptDialogLabelTitles:@{@"alertTitle":@"Notification",@"alertBtn":@"OK"}];
    _navigationDelegate = navDelegate;
    ...
    if (@available(iOS 11.0, *)) {
      ...
      UIProgressView *progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 0, 375, 2)];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if([keyPath isEqualToString:@"URL"]){
        if ([_currentUrl containsString:@"token="]!=[_webView.URL.absoluteString containsString:@"token="]) {
            NSString *arg = [_webView.URL.absoluteString containsString:@"token="]?@"true":@"false";
            NSDictionary *dic = @{@"callFlutter":@"flu_swipeChanged",@"state":arg};
            _currentUrl = _webView.URL.absoluteString;
            [_channel invokeMethod:@"flu_swipeChanged" arguments:dic];
        }
...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  }else if ([[call method] hasPrefix:@"ori_"]) {
      [MethodBridge callOrigin:call result:result andWebView:_webView];
  }else {
