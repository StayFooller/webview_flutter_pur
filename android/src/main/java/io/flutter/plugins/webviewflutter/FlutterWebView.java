...
//    KeyboardUtils.fixAndroidBug5497(WebViewFlutterPlugin.activity);//解决Android键盘遮挡问题
    ...
    webView.setOnTouchListener(new View.OnTouchListener() {
      ...
      public boolean onTouch(View v, MotionEvent event) {
        ...
        if(event.getAction() == MotionEvent.ACTION_UP) {
          ...
          y2 = event.getY();
          if(y1 - y2 > 50) {
            Log.i(TAG, "onTouch: 向上滑");
          } else if(y2 - y1 > 50) {
            Log.i(TAG, "onTouch: 向下滑");
          } else if(x1 - x2 > 50) {
            Log.i(TAG, "onTouch: 向左滑");
          } else if(x2 - x1 > 50) {
            Log.i(TAG, "onTouch: 向右滑");
          }else{
...
//        list.add(ordersn);
  ...
  private void choosePicture(WebChromeClient.FileChooserParams fileChooserParams) {
    ...
    Intent chooserIntent = Intent.createChooser(contentSelectionIntent, "请选择图片");
